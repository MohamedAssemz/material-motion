...
import { ArrowLeft, Package, Box, Loader2, QrCode, Search, CheckSquare, Zap, CheckCircle } from "lucide-react";
...
export default function OrderPackaging() {
    ...
    try {
      const [orderRes, batchesRes, completedRes] = await Promise.all([
        ...
        supabase
          .from("order_batches")
          .select(
            "id, qr_code_data, current_state, quantity, product_id, order_item_id, box_id, manufacturing_machine_id, finishing_machine_id, packaging_machine_id, production_date, from_extra_state, is_special, product:products(id, name_en, name_ar, sku, needs_packing, color_en, color_ar)",
          )
          .eq("order_id", id)
  ...
  batches
    .filter((b) => b.current_state === "ready_for_packaging" && b.box_id)
    .forEach((batch) => {
  ...
  // Group in_packaging by order_item_id to keep different sizes/colors as separate entries
  const inPackagingGroups: OrderItemGroup[] = [];
  const orderItemGroupMap = new Map<string, OrderItemGroup>();
  batches
    .filter((b) => b.current_state === "in_packaging")
    .forEach((batch) => {
  ...
  const handleAcceptBoxes = async () => {
    ...
    try {
      ...
      const batchIds = batches
        .filter((b) => b.current_state === "ready_for_packaging" && b.box_id && selectedBoxes.has(b.box_id))
        .map((b) => b.id);
      // Clear box_id when receiving - boxes become available again
      await supabase
        .from("order_batches")
        .update({
          current_state: "in_packaging",
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" onClick={() => navigate("/queues/packaging")}>
          ...
          <ProductionRateSection
            batches={[
              ...
              ...extraBatchesForRate.map((eb) => ({
                ...
                machine_id: eb.packaging_machine_id,
