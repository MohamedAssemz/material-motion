  ...
  const fetchOrders = async () => {
    try {
      const { data, error } = await supabase
        ...
        `)
        .or('current_state.eq.ready_for_boxing,current_state.eq.in_boxing', { foreignTable: 'batches' });
      ...
      const ordersWithCounts = (data || []).map((order: any) => ({
        ...
        ready_for_boxing_count: order.batches
        ...
        order.ready_for_boxing_count > 0 || order.boxing_count > 0
      );
