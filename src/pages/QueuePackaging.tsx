...
import { Package, Search, CalendarIcon, X } from 'lucide-react';
...
const PACKAGING_ACTIVE_STATES = ['ready_for_packaging', 'in_packaging'];
const PACKAGING_PAST_STATES = ['ready_for_boxing', 'in_boxing', 'ready_for_shipment', 'shipped'];
    ...
    try {
      ...
      orderBatchesData?.forEach(b => {
        if (b.order_id) {
          ...
          if (PACKAGING_ACTIVE_STATES.includes(b.current_state)) activeOrderIds.add(b.order_id);
          if (PACKAGING_PAST_STATES.includes(b.current_state)) orderHasPastBatches.add(b.order_id);
        }
      });
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabStatus)} className="space-y-4">
        ...
        <TabsContent value="active">
          <Card><CardContent className="pt-6">
            ...
            : filteredActive.length === 0 ? <div className="text-center py-8 text-muted-foreground">{t('queue.no_active_packaging')}</div>
        ...
        <TabsContent value="completed">
          <Card><CardContent className="pt-6">
            {loading ? <div className="flex justify-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div></div>
            : filteredCompleted.length === 0 ? <div className="text-center py-8 text-muted-foreground">{t('queue.no_completed_packaging')}</div>
